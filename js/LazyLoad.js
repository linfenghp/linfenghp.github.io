...
LazyLoad.prototype = {
	...
	_initContainer: function(container) {
		...
		//获取容器位置参数函数
		this._getContainerRect = isWindow && ("innerHeight" in window) ? function() {
			return {
				"left": 0,
		...
		//设置获取scroll值函数
		this._getScroll = isWindow ? function() {
			return {
...
$$.extend(ImagesLazyLoad.prototype, {
	...
	_initialize: function(options) {
		...
		this._elems = $$A.filter(opt.images || this._container.getElementsByTagName("img"),
			filter);
		// 判断属性是否已经加载的方法
